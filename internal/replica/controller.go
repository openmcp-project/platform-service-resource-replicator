	...
	for _, targetDef := range rr.Object.GetSpec().Targets {
		...
		for _, targetCluster := range matchedClusters {
			...
			if logClusterName == string(provider.HostingPlatformCluster) {
				logClusterName = "<hosting-platform-cluster>"
			}
			...
			for _, targetNamespace := range targetNamespaces {
				...
				trackedInStatus := rr.Object.GetStatus().Replicas.Contains(metav1.GroupVersionKind(renderedGVK), repv1alpha1.CreatedResource{
					Cluster: clusterStatusRef,
